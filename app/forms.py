...
class EditProfileForm(FlaskForm):
    ...
    def validate_username(self, username):
        if username.data != self.original_username:
            user = User.query = User.query.filter_by(username=self.username.data).first()
            if user is not None:
...
class CreateArticle(FlaskForm):
    tea_name = StringField('Name of the tea', validators=[
        DataRequired(), Length(min=1, max=64)])
    title = StringField('Article title', validators=[
        DataRequired(), Length(min=1, max=64)])
    text = TextAreaField('Tell about tea', validators=[DataRequired()])
